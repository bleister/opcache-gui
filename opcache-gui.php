...
class OpCacheService
{
	...
	protected function time_since($time,$original,$text='ago') {	
		$time =  $time - $original; 
			...
			$overview = array_merge(
				$status['memory_usage'], $status['opcache_statistics'], [
					...
					'readable' => [
						...
						'uptime' => ($status['opcache_statistics']['last_restart_time'] == 0) 
									? $this->time_since(time(),$status['opcache_statistics']['start_time'])
									: $this->time_since(time(),$status['opcache_statistics']['last_restart_time']),
					]
				]
			);
		}
			...
			$.ajax({
				...
				success: function(data) {
					...
					generalInfoObj.setState({
						...
						uptime : opstate.overview.readable.uptime
					});
					filesObj.setState({
						data : opstate.files,
	...
	var GeneralInfo = React.createClass({
		getInitialState: function () {
			return {
				...
				uptime: opstate.overview ? opstate.overview.readable.uptime : null
			};
		},
		render: function () {
			...
			var upTime = this.state.reset ? React.createElement(
				"tr",
				null,
				React.createElement(
					"td",
